...
var clickcoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in clickcoreDefaults)
    if (clickcoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = clickcoreDefaults[i];
